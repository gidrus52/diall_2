...
app.post('/obtainUserevents', async (req, res, next) => {
    ...
    try {
        const response = await listAuthEvents(req.body.username);
...
app.post('/createUser', async (req, res, next) => {
    ...
    try {
        const response = await createUser(req.body.email, req.body.name, req.body.password, req.body.group);
        res.status(200).json(response);
    } catch (err) {
        next(err);
    }
