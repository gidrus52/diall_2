...
async function createUser(email, name, password, group) {
  const params = {
    UserPoolId: userPoolId,
  ...
  try {
    ...
    if (group)
    {
      await addUserToGroup(result.User.Username, group)
    }
    return {
      result:result
    };
...
async function listAuthEvents(name) {
  const params = {
    UserPoolId: userPoolId,
    Username: name
  };
  console.log(params);
  try {
    const result = await cognitoIdentityServiceProvider.adminListUserAuthEvents(params).promise();
    console.log(result)
    return {
      result:result
    };
